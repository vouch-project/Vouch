...
export const loginWithWallet = async (signer: ethers.Signer): Promise<string> => {
  const address = await signer.getAddress();
  const existingToken = localStorage.getItem(JWT_STORAGE_KEY);
  if (existingToken && isJwtValid(existingToken, address)) {
    return existingToken;
  }

  const { nonce } = (await axiosApi.get(`/auth/nonce?address=${address}`)).data;
