...
describe('VouchVault', function () {
  ...
  describe('createLoan', function () {
    it('Should create a loan with collateral', async function () {
      ...
      expect(loan.collateralToken).to.equal(ethers.ZeroAddress);
      expect(loan.collateralAmount).to.equal(sentCollateral);
      expect(loan.active).to.equal(true); // active
      expect(await vault.deposits(owner.address)).to.equal(0);
      expect(await vault.lockedEthCollateral(owner.address)).to.equal(sentCollateral);
      expect(
        await (async () => {
          const _ld = await vault.loans(0);
          return _ld.collateralAmount - _ld.collateralReleased;
        })(),
      ).to.equal(sentCollateral);

      const locked = await vault.loans(0);
      expect(locked.collateralToken).to.equal(ethers.ZeroAddress);
      expect(locked.collateralAmount).to.equal(sentCollateral);
      expect(locked.collateralLocked).to.equal(true);
    ...
    it('Should fail if fund window is zero', async function () {
      ...
      ).to.be.revertedWithCustomError(vault, 'InvalidFundWindow');
    });
    ...
    it('reverts when the interest rate exceeds 100% (10000 bps cap)', async function () {
      ...
      ).to.be.revertedWithCustomError(vault, 'InvalidInterestRate');
    ...
    it('Should not allow withdrawing active ETH loan collateral', async function () {
      ...
      await expect(vault.withdraw(collateral)).to.be.revertedWithCustomError(vault, 'InsufficientBalance');
    });
    ...
    it('Should lock ERC20 collateral per loan', async function () {
      ...
      const loan = await vault.loans(0);
      expect(loan.borrower).to.equal(owner.address);
      expect(loan.collateralToken).to.equal(await token.getAddress());
      expect(loan.collateralAmount).to.equal(collateral);
      expect(loan.active).to.equal(true);

      const locked = await vault.loans(0);
      expect(locked.collateralToken).to.equal(await token.getAddress());
      expect(locked.collateralAmount).to.equal(collateral);
      expect(locked.collateralLocked).to.equal(true);
    });
  ...
  describe('fundLoan', function () {
    ...
    it('Should fail if loan does not exist (inactive)', async function () {
      ...
      await expect(
        vault.connect(lender).fundLoan(999, { value: ethers.parseEther('1.0') }),
      ).to.be.revertedWithCustomError(vault, 'LoanNotActive');
    });
    ...
    it('Should fail if loan is already funded', async function () {
      ...
      await expect(vault.connect(lender).fundLoan(0, { value: principal })).to.be.revertedWithCustomError(
        vault,
        'LoanAlreadyFunded',
      );
    });
    ...
    it('Should fail if borrower tries to fund own loan', async function () {
      ...
      await expect(
        vault.connect(borrower).fundLoan(0, { value: ethers.parseEther('1.0') }),
      ).to.be.revertedWithCustomError(vault, 'BorrowerCannotFundOwnLoan');
    });

    it('Should fail if msg.value does not equal requestedPrincipalAmount', async function () {
      const { vault, lender, principal } = await deployWithLoan();
      await expect(vault.connect(lender).fundLoan(0, { value: principal - 1n })).to.be.revertedWithCustomError(
        vault,
        'PrincipalAmountMismatch',
      );
      await expect(vault.connect(lender).fundLoan(0, { value: principal + 1n })).to.be.revertedWithCustomError(
        vault,
        'PrincipalAmountMismatch',
      );
      await expect(vault.connect(lender).fundLoan(0, { value: 0 })).to.be.revertedWithCustomError(
        vault,
        'PrincipalAmountMismatch',
      );
    });
    ...
    it('Should not affect collateral tracking when funded', async function () {
      ...
      expect(await vault.lockedEthCollateral(borrower.address)).to.equal(collateral);
      expect(
        await (async () => {
          const _ld = await vault.loans(0);
          return _ld.collateralAmount - _ld.collateralReleased;
  ...
  describe('fundLoanWithERC20', function () {
    it('Should fund an ERC20-principal loan and transfer tokens to borrower', async function () {
      ...
      const tx = await vault.connect(lender).fundLoan(0);
    ...
    it('Should fail if loan requests native ETH principal', async function () {
      ...
      await expect(vault.connect(lender).fundLoan(0)).to.be.revertedWithCustomError(vault, 'PrincipalAmountMismatch');
    });
    ...
    it('reverts when funding after the fund window has passed', async function () {
      ...
      await expect(vault.connect(lender).fundLoan(0)).to.be.revertedWithCustomError(vault, 'FundingWindowPassed');
    });
  });
  ...
  describe('repayLoan (ETH principal)', function () {
    ...
    it('applies partial payment to interest first, releases collateral by principal', async function () {
      ...
      const tx = await vault.connect(borrower).repayLoan(0, 0n, { value: interest });
      await tx.wait();
      const loan = await vault.loans(0);
    ...
    it('full repayment after accrual closes loan and returns all collateral', async function () {
      ...
      await vault.connect(borrower).repayLoan(0, 0n, { value: totalDue });
      const loan = await vault.loans(0);
    ...
    it('Should repay loan in full and return all collateral to borrower', async function () {
      ...
      const tx = await vault.connect(borrower).repayLoan(0, 0n, { value: totalDue });
    ...
    it('Should unlock collateral tracking after full repayment', async function () {
      ...
      expect(await vault.lockedEthCollateral(borrower.address)).to.equal(0);
      const locked = await vault.loans(0);
    ...
    it('Should work with zero interest rate', async function () {
      ...
      const lenderBefore = await ethers.provider.getBalance(lender.address);
      const tx = await vault.connect(borrower).repayLoan(0, 0n, { value: principal });
      await expect(tx)
        .to.emit(vault, 'LoanRepaid')
    ...
    it('Should fail if payment exceeds remaining balance', async function () {
      ...
      await expect(vault.connect(borrower).repayLoan(0, 0n, { value: totalDue + 1n })).to.be.revertedWithCustomError(
        vault,
        'PaymentExceedsAmountOwed',
      );
    });
    ...
    it('Should fail if payment is zero', async function () {
      ...
      await expect(vault.connect(borrower).repayLoan(0, 0n, { value: 0 })).to.be.revertedWithCustomError(
        vault,
        'ZeroValue',
      );
    });
    ...
    it('Should fail if called by non-borrower', async function () {
      ...
      await expect(vault.connect(lender).repayLoan(0, 0n, { value: totalDue })).to.be.revertedWithCustomError(
        vault,
        'OnlyBorrower',
      );
    });
    ...
    it('Should fail if loan is not funded', async function () {
      ...
      await expect(vault.connect(borrower).repayLoan(0, 0n, { value: collateral })).to.be.revertedWithCustomError(
        vault,
        'LoanNotFunded',
      );
    });
    ...
    it('Should fail on double repayment', async function () {
      ...
      await expect(vault.connect(borrower).repayLoan(0, 0n, { value: totalDue })).to.be.revertedWithCustomError(
        vault,
        'LoanAlreadyRepaid',
      );
    });
    ...
    it('Should fail if loan has ERC20 principal', async function () {
      ...
      await token.transfer(lender.address, principalAmount);
      await token.connect(lender).approve(await vault.getAddress(), principalAmount);
      await vault.connect(lender).fundLoan(0);

      await expect(
        vault.connect(borrower).repayLoan(0, 0n, { value: ethers.parseEther('1.1') }),
      ).to.be.revertedWithCustomError(vault, 'InvalidAmount');
    });
    ...
    describe('partial repayments', function () {
      it('Should release proportional collateral on each partial payment', async function () {
        ...
        const tx = await vault.connect(borrower).repayLoan(0, 0n, { value: half });
        const receipt = await tx.wait();
        const gasUsed = receipt!.gasUsed * receipt!.gasPrice;
      ...
      it('Should report remaining locked collateral via view helpers after a partial payment', async function () {
        ...
        await vault.connect(borrower).repayLoan(0, 0n, { value: half });
        ...
        ).to.equal(expectedStillLocked);
        const locked = await vault.loans(0);
        expect(locked.collateralAmount - locked.collateralReleased).to.equal(expectedStillLocked);
        expect(locked.collateralLocked).to.equal(true); // still locked (loan not fully repaid)
      });
      ...
      it('Should forward each partial payment to the lender', async function () {
        ...
        await vault.connect(borrower).repayLoan(0, 0n, { value: third });
      ...
      it('Should recover all collateral across multiple partial payments with no dust lost', async function () {
        ...
        await vault.connect(borrower).repayLoan(0, 0n, { value: p1 });
        await vault.connect(borrower).repayLoan(0, 0n, { value: p2 });

        // Before final payment: some collateral already released
        expect(await vault.lockedEthCollateral(borrower.address)).to.be.lessThan(collateral);

        const borrowerEthBefore = await ethers.provider.getBalance(borrower.address);
        const tx = await vault.connect(borrower).repayLoan(0, 0n, { value: p3 });
        const receipt = await tx.wait();
        const gasUsed = receipt!.gasUsed * receipt!.gasPrice;
        const borrowerEthAfter = await ethers.provider.getBalance(borrower.address);
      ...
      it('Should fail on a payment that would exceed the remaining balance', async function () {
        ...
        await expect(vault.connect(borrower).repayLoan(0, 0n, { value: remaining + 1n })).to.be.revertedWithCustomError(
          vault,
          'PaymentExceedsAmountOwed',
        );
      });
      ...
      it('Should allow paying exactly the remaining balance to close the loan', async function () {
        ...
        const remaining = totalDue - half;
        const tx = await vault.connect(borrower).repayLoan(0, 0n, { value: remaining });
      ...
      it('releases collateral strictly by principal across the interest->principal boundary', async function () {
        ...
        await vault.connect(borrower).repayLoan(0, 0n, { value: payment1 });
        ...
        await vault.connect(borrower).repayLoan(0, 0n, { value: payment2 });
      ...
      it('does not revert when interest accrues further between an early principal payment and a later small payment', async function () {
        ...
        await expect(vault.connect(borrower).repayLoan(0, 0n, { value: payment2 })).to.not.be.reverted;
  ...
  describe('repayLoanWithERC20 (ERC20 principal)', function () {
    ...
    it('Should repay ERC20 loan in full, forward tokens to lender, return ETH collateral', async function () {
      ...
      const tx = await vault.connect(borrower).repayLoan(0, totalDue);
      const receipt = await tx.wait();
      const gasUsed = receipt!.gasUsed * receipt!.gasPrice;
    ...
    it('Should repay ERC20 loan with ERC20 collateral and return ERC20 collateral', async function () {
      ...
      await vault.connect(borrower).repayLoan(0, totalDue);
    ...
    it('Should fail if called by non-borrower', async function () {
      ...
      await expect(vault.connect(lender).repayLoan(0, totalDue)).to.be.revertedWithCustomError(vault, 'OnlyBorrower');
    });
    ...
    it('Should fail on double repayment', async function () {
      ...
      await token.connect(borrower).approve(await vault.getAddress(), totalDue);
      await expect(vault.connect(borrower).repayLoan(0, totalDue)).to.be.revertedWithCustomError(
        vault,
        'LoanAlreadyRepaid',
      );
    });
    ...
    it('Should fail if loan has ETH principal', async function () {
      ...
      await expect(vault.connect(borrower).repayLoan(0, principal)).to.be.revertedWithCustomError(vault, 'ZeroValue');
    });
    ...
    it('Should fail if borrower has insufficient token allowance', async function () {
      ...
      await expect(vault.connect(borrower).repayLoan(0, totalDue)).to.be.reverted;
    });
    ...
    it('Should fail if payment exceeds remaining balance', async function () {
      ...
      await expect(vault.connect(borrower).repayLoan(0, totalDue + 1n)).to.be.revertedWithCustomError(
        vault,
        'PaymentExceedsAmountOwed',
      );
    });
    ...
    describe('partial repayments', function () {
      it('Should release proportional ERC20 collateral on partial payment and forward tokens to lender', async function () {
        ...
        const tx = await vault.connect(borrower).repayLoan(0, half);
      ...
      it('Should recover all ERC20 collateral across multiple partial payments with no dust lost', async function () {
        ...
        await vault.connect(borrower).repayLoan(0, p1);

        await token.connect(borrower).approve(await vault.getAddress(), p2);
        await vault.connect(borrower).repayLoan(0, p2);
        ...
        const tx = await vault.connect(borrower).repayLoan(0, p3);
        const receipt = await tx.wait();
        const gasUsed = receipt!.gasUsed * receipt!.gasPrice;
        const borrowerEthAfter = await ethers.provider.getBalance(borrower.address);
      ...
      it('ERC20: does not revert when interest accrues further between an early principal payment and a later small payment', async function () {
        ...
        // Pay slightly more than the accrued interest so some principal is credited.
        await vault.connect(borrower).repayLoan(0, payment1);
        ...
        await expect(vault.connect(borrower).repayLoan(0, payment2)).to.not.be.reverted;
  ...
  describe('outstanding-balance interest & minimum floor', function () {
    ...
    it('charges interest only on the outstanding principal after an early principal payment', async function () {
      ...
      await vault.connect(borrower).repayLoan(0, 0n, { value: firstPayment });
    ...
    it('keeps interest identical when no principal is repaid early (drip equals lump)', async function () {
      ...
      await ethers.provider.send('evm_increaseTime', [10 * 86400]);
      await ethers.provider.send('evm_mine', []);
      await vault.connect(borrower).repayLoan(0, 0n, { value: perDayFull * 10n });
    ...
    it('guarantees the lender a floor return when the borrower repays instantly', async function () {
      ...
      await vault.connect(borrower).repayLoan(0, 0n, { value: principal + floor });
  ...
  describe('cancelLoan', function () {
    ...
    it('reverts if a non-borrower tries to cancel', async function () {
      ...
      await expect(vault.connect(lender).cancelLoan(0)).to.be.revertedWithCustomError(vault, 'OnlyBorrower');
    });

    it('reverts if the loan is already funded', async function () {
      const { vault, borrower, lender, principal } = await deployUnfunded();
      await vault.connect(lender).fundLoan(0, { value: principal });
      await expect(vault.connect(borrower).cancelLoan(0)).to.be.revertedWithCustomError(vault, 'LoanAlreadyFunded');
    });
  ...
  describe('protocol fee', function () {
    ...
    it('lets the owner update the treasury but rejects the zero address', async function () {
      ...
      await expect(vault.setProtocolTreasury(ethers.ZeroAddress)).to.be.revertedWithCustomError(
        vault,
        'InvalidAddress',
      );
    });
  ...
  describe('pull-over-push payouts', function () {
    it('reverts withdrawPayments when nothing is credited', async function () {
      ...
      const vault = await upgrades.deployProxy(VouchVault, [owner.address], { kind: 'uups' });
      await vault.setScoreSigner(owner.address);
      await expect(vault.connect(lender).withdrawPayments(ethers.ZeroAddress)).to.be.revertedWithCustomError(
        vault,
        'NothingToWithdraw',
      );
    });
    ...
    it('falls back to a credit when a lender rejects ETH, without blocking repayment', async function () {
      ...
      await expect(vault.connect(borrower).repayLoan(0, 0n, { value: principal })).to.not.be.reverted;
    ...
    it('falls back to a credit when an ERC20 payout fails, and the lender can still claim', async function () {
      ...
      await vault.connect(borrower).repayLoan(0, principal);
  ...
  describe('expireLoan', function () {
    ...
    it('reverts if loan is still within fund window and no price feeds set', async function () {
      ...
      await expect(vault.expireLoan(0)).to.be.revertedWithCustomError(vault, 'LoanCannotBeExpired');
    });
    ...
    it('reverts if loan is already funded', async function () {
      ...
      await ethers.provider.send('evm_mine', []);
      await expect(vault.expireLoan(0)).to.be.revertedWithCustomError(vault, 'LoanAlreadyFunded');
    });
    ...
    it('reverts if loan is already expired (inactive)', async function () {
      ...
      await expect(vault.expireLoan(0)).to.be.revertedWithCustomError(vault, 'LoanNotActive');
    });
  ...
  describe('Chainlink & health factor', function () {
    ...
    it('setPriceFeed reverts if decimals_ exceeds 18', async function () {
      ...
      ).to.be.revertedWithCustomError(vault, 'InvalidDecimals');
    });
    ...
    it('getHealthFactor reverts if the price feed reports a stale round', async function () {
      ...
      await expect(vault.getHealthFactor(0)).to.be.revertedWithCustomError(vault, 'StaleRound');
    });
    ...
    it('getHealthFactor reverts if the price feed reports a future timestamp', async function () {
      ...
      await expect(vault.getHealthFactor(0)).to.be.revertedWithCustomError(vault, 'PriceTimestampInFuture');
    });
    ...
    it('getHealthFactor reverts if the price feed reports decimals > 18', async function () {
      ...
      await expect(vault.getHealthFactor(0)).to.be.revertedWithCustomError(vault, 'FeedDecimalsTooLarge');
    });
    ...
    it('fundLoanWithERC20 reverts when loan is undercollateralized at funding time', async function () {
      ...
      await mockToken.transfer(lender.address, principal);
      await mockToken.connect(lender).approve(await vault.getAddress(), principal);
      await expect(vault.connect(lender).fundLoan(0)).to.be.revertedWithCustomError(vault, 'LoanIsUndercollateralized');
    });
    ...
    it('liquidate (ETH-principal entry) reverts when called on an ERC20-principal loan', async function () {
      ...
      await expect(vault.liquidate(0, 0n, ethers.ZeroAddress)).to.be.revertedWithCustomError(
        vault,
        'LoanNotLiquidatable',
      );
    });
    ...
    describe('liquidate (ETH principal)', function () {
      it('reverts when called on an ERC20-principal loan', async function () {
        ...
        await expect(vault.liquidate(0, 0n, ethers.ZeroAddress, { value: principal })).to.be.revertedWithCustomError(
          vault,
          'InvalidAmount',
        );
      });
      ...
      it('reverts on unfunded loan', async function () {
        ...
        await expect(vault.liquidate(0, 0n, ethers.ZeroAddress, { value: collateral })).to.be.revertedWithCustomError(
          vault,
          'LoanNotFunded',
        );
      });
      ...
      it('expired ETH-principal loan: liquidator pays debt, lender paid, excess collateral returned, LoanLiquidated emitted', async function () {
        ...
        const tx = await vault.connect(liquidator).liquidate(0, 0n, ethers.ZeroAddress, { value: principal });
    ...
    describe('liquidateWithERC20 (ERC20 principal)', function () {
      it('reverts on a healthy non-expired loan', async function () {
        ...
        await expect(vault.liquidate(0, ethers.parseUnits('2', 18), ethers.ZeroAddress)).to.be.revertedWithCustomError(
          vault,
          'LoanNotLiquidatable',
        );
      });
      ...
      it('reverts when wrong entry point used (ETH-principal loan)', async function () {
        ...
        await expect(vault.liquidate(0, collateral, ethers.ZeroAddress)).to.be.revertedWithCustomError(
          vault,
          'LoanNotFunded',
        );
      });
      ...
      it('reverts on already-repaid loan', async function () {
        ...
        await vault.connect(borrower).repayLoan(0, principal);
        await expect(vault.liquidate(0, principal, ethers.ZeroAddress)).to.be.revertedWithCustomError(
          vault,
          'LoanAlreadyRepaid',
        );
      });
      ...
      it('healthy-but-liquidatable: liquidator pays full debt, seizes debt*(1+bonus) collateral, borrower gets excess', async function () {
        ...
        const tx = await vault.connect(liquidator).liquidate(0, debt, ethers.ZeroAddress);
        const receipt = await tx.wait();
      ...
      it('protocol fee on interest taken from lender payout on healthy close', async function () {
        ...
        await vault.connect(liquidator).liquidate(0, debt, ethers.ZeroAddress);
      ...
      it('underwater: liquidator pays collateralValue/(1+bonus), lender eats shortfall, no protocol fee', async function () {
        ...
        const tx = await vault.connect(liquidator).liquidate(0, expectedPay + 1n, ethers.ZeroAddress);
        const receipt = await tx.wait();
      ...
      it('ceiling too low reverts with Exceeds max payment', async function () {
        ...
        await expect(vault.connect(liquidator).liquidate(0, tooLow, ethers.ZeroAddress)).to.be.revertedWithCustomError(
          vault,
          'ExceedsMaxPayment',
        );
      });
      ...
      it('expired loan is liquidatable regardless of health factor', async function () {
        ...
        await expect(vault.connect(liquidator).liquidate(0, debt, ethers.ZeroAddress)).to.emit(vault, 'LoanLiquidated');
      ...
      it('explicit collateralRecipient receives seized collateral instead of msg.sender', async function () {
        ...
        const tx = await vault.connect(liquidator).liquidate(0, debt, treasury.address);
        const receipt = await tx.wait();
        const gasUsed = receipt!.gasUsed * receipt!.gasPrice;
      ...
      it('mismatched decimals: 18-dec ETH collateral, 6-dec USDC principal', async function () {
        ...
        const tx = await vault.connect(liquidator).liquidate(0, principalUsdc, ethers.ZeroAddress);
        const receipt = await tx.wait();
        const gasUsed = receipt!.gasUsed * receipt!.gasPrice;
  ...
  describe('lendOffer', function () {
    ...
    it('createLendOffer: locks ETH principal, emits LendOfferCreated', async function () {
      ...
      const tx = await vault
        .connect(lender)
        .createLendOffer(ethers.ZeroAddress, 0n, RATIO, TRUSTED, SCORE_THRESH, LTV, RATE, DURATION, WINDOW, {
          value: principal,
        });
    ...
    it('acceptLendOffer: borrower posts ETH collateral, creates loan, emits LendOfferAccepted', async function () {
      ...
      const tx = await vault
        .connect(borrower)
        .acceptLendOffer(1, ethers.ZeroAddress, 0n, 0, 0, '0x', { value: collateral });
    ...
    it('acceptLendOffer: reverts if collateral value below required ratio', async function () {
      ...
      await expect(
        vault
          .connect(borrower)
          .acceptLendOffer(1, ethers.ZeroAddress, 0n, 0, 0, '0x', { value: ethers.parseEther('1.0') }),
    ...
    it('createLendOfferWithERC20: locks ERC20 principal, emits LendOfferCreated', async function () {
      ...
      const tx = await vault
        .connect(lender)
        .createLendOffer(usdcAddress, principal, RATIO, TRUSTED, SCORE_THRESH, LTV, RATE, DURATION, WINDOW);
    ...
    it('acceptLendOfferWithERC20: borrower posts ERC20 collateral, creates loan', async function () {
      ...
      const tx = await vault.connect(borrower).acceptLendOffer(1, wbtcAddress, collateral, 0, 0, '0x');
      await expect(tx).to.emit(vault, 'LendOfferAccepted').withArgs(1, 0, borrower.address);
    ...
    it('acceptLendOffer: trusted ratio applied when valid attestation provided', async function () {
      ...
      await vault
        .connect(lender)
        .createLendOffer(ethers.ZeroAddress, 0n, 20000, TRUSTED_RATIO, SCORE_MIN, MAX_LTV, RATE, DURATION, WINDOW, {
      ...
      const tx = await vault
        .connect(borrower)
        .acceptLendOffer(1, ethers.ZeroAddress, 0n, score, expiry, sig, { value: collateral });
      await expect(tx).to.emit(vault, 'LendOfferAccepted').withArgs(1, 0, borrower.address);
      ...
      await expect(
        vault.connect(borrower).acceptLendOffer(2, ethers.ZeroAddress, 0n, 0, 0, '0x', { value: collateral }),
      ).to.be.revertedWithCustomError(vault, 'LoanIsUndercollateralized');
    });
  });
