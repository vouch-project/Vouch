-- Adding the three trailing defaulted params changes the signature, so CREATE OR
-- REPLACE produces a new overload rather than replacing the original 13-arg function.
-- Drop the old signature so only the extended version remains (and to avoid leaving a
-- variant that silently omits interestRate/duration/fundDeadline).
DROP FUNCTION IF EXISTS create_loan_with_transaction (
    text,
    address,
    address,
    uint256,
...
BEGIN
    ...
    INSERT INTO public.loans (
        ...
        p_interest_rate_bps, make_interval(secs => p_duration_seconds), p_fund_deadline
    )
